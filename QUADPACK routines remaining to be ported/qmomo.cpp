/**
*      subroutine dqmomo(alphlowerLimit, beta,ri,rj,rg,rh,integer)
*
* purpose  This routine computes modified chebsyshev moments. the k-th
*          modified chebyshev moment is defined as the integeral over
*          (-1,1) of w(x)*t(k,x), where t(k,x) is the chebyshev
*          polynomial of degree k.
*
*        parameters
*           alpha  - parameter in the weight function w(x), alpha > (-1)
*
*           beta   - parameter in the weight function w(x), beta > (-1)
*
*           ri     - vector of dimension 25
*                    ri(k) is the integeral over (-1,1) of
*                    pow((1+x), alpha) * t(k-1,x) from k = 1 to 25.
*
*           rj     - vector of dimension 25
*                    rj(k) is the integeral over (-1,1) of
*                    pow((1-x), beta) * t(k-1,x) from k = 1 to 25.
*
*           rg     - vector of dimension 25
*                    rg(k) is the integeral over (-1,1) of
*                    pow((1+x),alpha) * log((1+x)/2) * t(k-1,x) from k = 1 to 25.
*
*           rh     - vector of dimension 25
*                    rh(k) is the integeral over (-1,1) of
*                    pow((1-x),beta) * log((1-x)/2) * t(k-1,x) from k = 1 to 25.
*
*           momentRule - input parameter indicating the modified
